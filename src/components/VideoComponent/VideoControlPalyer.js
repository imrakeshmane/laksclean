...
import {
    ...
    Image,
    View,
    Text, TouchableOpacity, StatusBar, Dimensions
} from 'react-native';
...
export default class CustomVideoPlayer extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            // Controls
            isFullscreen: true,
            // isFullscreen: this.props.isFullScreen || this.props.resizeMode === 'contain' || false,
    ...
    _toggleFullscreen() {
        let state = this.state;
        // console.log('Ravi=' + state.isFullscreen);
        ...
        if (state.isFullscreen) {
            ...
            this.props.setCheckScreen(true);
            state.resizeMode = 'contain';
        } else {
            state.isFullscreen = !state.isFullscreen;
            Orientation.lockToPortrait();
            this.props.setCheckScreen(false);
    ...
    calculateTime() {
        ...
        if ((Math.floor(this.state.currentTime % 60).toFixed(0) === 1)) {
    ...
    formatTime(time = 0) {
        ...
        const formattedHours = Math.floor(time / 3600).toFixed(0);
        const secZero = formattedSeconds < 10 ? '0' : '';
        const minZero = formattedMinutes < 10 ? '0' : '';
        const hrZero = formattedHours < 10 ? '' : '';

        return `${symbol}${hrZero}${formattedHours}:${minZero}${formattedMinutes}:${secZero}${formattedSeconds}`;
    }

    formatTotalTime = () => {
        const formattedMinutes = Math.floor(this.state.duration / 60).toFixed(0);
        const formattedSeconds = Math.floor(this.state.duration % 60).toFixed(0);
        const formattedHours = Math.floor(this.state.duration / 3600).toFixed(0);
        const secZero = formattedSeconds < 10 ? '0' : '';
        const minZero = formattedMinutes < 10 ? '0' : '';
        const hrZero = formattedHours < 10 ? '' : '';

        return `${hrZero}${formattedHours}:${minZero}${formattedMinutes}:${secZero}${formattedSeconds}`;
    }
    ...
    componentDidMount() {
        ...
        this.mounted = true;
        // Orientation.addOrientationListener(this.getOrientation);
        this.getOrientation();
        this.setState(state);
    }
    ...
    renderTopControls() {
        ...
        return (
            <Animated.View
                ...
                <View style={{ flex: 1, backgroundColor: '#ffffff00', justifyContent: 'center', alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 5 }}>
                    <TouchableOpacity onPress={() => this.doubleTabTest(2)} style={{ width: '30%', height: '100%', backgroundColor: '#ffffff00', justifyContent: 'center', alignItems: 'center' }}>
                        {this.state.backword && <View>
                            <Image
                                source={require('../../assets/VideoPlayerImages/NewIcon/backward.gif')}
                                style={{ height: 20, width: 80 }} resizeMode='contain' />

                            <Text style={{ color: 'white' }}>{'10 Seconds'}</Text>
                        </View>
                        }

                    </TouchableOpacity>
                    {this.renderPlayPause()}
                    <TouchableOpacity onPress={() => this.doubleTabTest(1)} style={{ width: '30%', height: '100%', backgroundColor: '#ffffff00', justifyContent: 'center', alignItems: 'center' }}>
                        {this.state.forword && <View>
                            <Image
                                source={require('../../assets/VideoPlayerImages/NewIcon/Forward.gif')}
                                style={{ height: 20, width: 80 }} resizeMode='contain' />
                            <Text style={{ color: 'white' }}>{'10 Seconds'}</Text>
                        </View>
                        }
    ...
    hideForwordBackwordIcon = () => {
        let state = this.state;
        state.forword = false;
        state.backword = false;
    }
    doubleTabTest = (evt) => {
        ...
        if (delta < 300) {
            state.showControls = false;
            this.methods.toggleControls();
            evt == 1 ? state.forword = true : state.backword = true
            evt == 1 ? this.seekTo(time + 10) : this.seekTo(time - 10);

            setTimeout(() => { this.hideForwordBackwordIcon() }, 500);
            ...
            <View style={styles.volume.container} >
                ...
                <View
                    ...
                    <Image
                        ...
                        source={require('../../assets/VideoPlayerImages/volume.png')}
                    />
                </View>
            </View>
        );
    ...
    renderFullscreen() {
        let source =
            this.state.isFullscreen === true
                ? require('../../assets/VideoPlayerImages/NewIcon/expand.png')
                : require('../../assets/VideoPlayerImages/NewIcon/shrink.png');
        return this.renderControl(
            <Image source={source} />,
            this.methods.toggleFullscreen,
                    ...
                    <View
                        style={[
                            styles.seekbar.fill,
                            {
                                ...
                                backgroundColor: this.props.seekColor || '#00adf6',
                            },
                        ]}
                        pointerEvents={'none'}
                    />
    ...
    renderPlayPause() {
        let source =
            this.state.paused === true
                ? require('../../assets/VideoPlayerImages/NewIcon/Play.png')
                : require('../../assets/VideoPlayerImages/NewIcon/Pause.png');
        return this.renderControl(
            <Image source={source} />,
            this.methods.togglePlayPause,
    ...
    totalTimer() {
        return this.renderControl(
            < Text style={styles.controls.timerText2} >{this.formatTotalTime()}</Text >,
            this.methods.toggleTimer,
            styles.controls.timer,
        );
    }
    ...
    renderLoader() {
        ...
        // return <View style={[styles.loader.container, { backgroundColor: 'red', opacity: 4 }]}>
        ...
        if (this.state.error) {
            return (
                <View style={styles.error.container}>
                    <Image
                        source={require('../../assets/VideoPlayerImages/error-icon.png')}
        ...
        return (
            ...
            <TouchableOpacity
                ...
                {/* <ScreenOrientation
                    ...
                    onDeviceChange={orientation => alert('rr=', orientation)}
                /> */}
...
const styles = {
    ...
    controls: StyleSheet.create({
        ...
        timerText: {
            ...
            color: '#FFF',
            fontSize: 11,
            marginBottom: 17, width: 70, alignSelf: 'auto', left: 5, paddingLeft: 0
        },
        timerText2: {
            backgroundColor: 'transparent',
            color: '#FFF',
            fontSize: 11,
            textAlign: 'right', marginBottom: 17
        },
    }),
    volume: StyleSheet.create({
        ...
        handle: {
            ...
            marginTop: -24,
            marginLeft: -24,
            padding: 21, backgroundColor: '#ffffff00'
        },
        icon: {
            marginLeft: 7,
        },
